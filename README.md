...
截图识别、队伍存储和伤害计算均在设备本地完成。只有用户点击解析公开队伍码时，App 才会把该十位公开码发送给配置的解析服务，并只接收规范化后的公开队伍数据。项目不会修改、注入或 Hook 游戏进程，不读取游戏内存，不拦截游戏网络，也不会自动操作游戏。
...
解析服务不可用与公开码不存在会分别提示；保存六个对手配置采用一次性写入，不会留下半支导入结果。
...
| 网络访问 | 手动检查更新时访问 GitHub；用户主动解析队伍码时仅向配置的解析服务发送该公开码并接收公开队伍，不上传截图、本地队伍库或计算数据；系统备份开启时，Android 可加密同步所选 JSON 数据 |
...
公开队伍码解析地址的构建参数、HTTPS 响应结构和服务端安全边界见 [公开队伍码解析服务契约](docs/team_code_resolver_contract_zh.md)。
