...
状态：需求已确认，方案已按当前 `v1.1.0` 代码重审，尚未实现
...
这说明系统和 Manifest 策略没有直接禁止播放音频捕获，但仍不能证明游戏实际播放器输出一定允许被录制。播放器可以设置更严格的捕获策略，所以必须以“真机取得非静音 PCM”为准。
...
- [ ] 重新核对 RMX3820、OPD2409 的 Android、游戏版本和游戏 UID。
- [ ] 查询 H.264/AAC 编码器能力和硬件实现。
- [ ] 做 10 秒单应用视频探针，检查悬浮球、菜单、面板、状态栏、通知和其他应用是否被排除。
- [ ] 做游戏音频 PCM 探针：游戏有声、游戏静音、其他应用播放三组对照。
...
- [ ] 在 EGL 路由中增加按需原分辨率 FBO 读回。
- [ ] 把读回 Bitmap 接到现有 `captureFrame()` 后半段识别逻辑。
- [ ] 保留菜单冻结、悬浮球隐藏、人工核对和伤害面板行为。
- [ ] 识别时编码持续运行，识别失败不影响录屏。
- [ ] 给读回、OCR、图像匹配和编码设置独立线程/队列。

退出条件：

- 完整走通“识别双方阵容 -> 核对 -> 打开伤害面板 -> 结束并保存”；
- 回放不出现核对页、伤害面板或悬浮球；
- OCR 输入分辨率和 `v1.1.0` 一致，现有识别回归不下降；
- 识别期间无明显长黑帧、音频断层或编码器超时。
...
| 风险 | 当前认识 | 处理 |
| --- | --- | --- |
| 游戏播放器禁止播放捕获 | Manifest/系统包标志允许，但播放器仍可拒绝 | Phase 0 真机 PCM 探针；不 Hook、不改用麦克风 |
