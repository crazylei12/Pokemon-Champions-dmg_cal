...
- 烧伤。
- 双打范围伤害。
- 当前上场特性联动：从双方各自最多两个场上槽位统一派生，详见 4.1.1。
- 目标保护状态，后续支持。

#### 4.1.1 当前上场特性的自动派生边界

伤害请求按当前计算方向区分攻击方、目标与双方队友，但扫描范围始终是双方真实/假设的当前上场槽位，不读取后排。对方特性只使用用户选定的当前配置；仅当该形态在规则数据中只有一个合法特性时，才允许自动补全，不能从多个可能特性中猜测。

| 类型 | 自动处理的特性 | 计算语义 |
| --- | --- | --- |
| 全场招式修正 | 妖精气场、暗黑气场、气场破坏 | 任一方持有者都影响对应属性；气场破坏把气场倍率反转 |
| 全场能力修正 | 灾祸之简、灾祸之剑、灾祸之鼎、灾祸之玉 | 分别修正攻击、防御、特攻、特防；同名灾祸持有者不受该项影响 |
| 攻击方队友增益 | 蓄电池、能量点、钢之意志、花之礼 | 只由同侧另一只场上宝可梦提供；花之礼还要求晴天 |
| 防守方队友增益 | 友情防守、花之礼 | 只保护同侧另一只场上宝可梦，不保护持有者自己；花之礼还要求晴天且只修正特防 |
| 组合增益 | 正电、负电 | 按 Champions 规则检查场上相反特性，并给当前攻击者启用特攻增益 |
| 改换目标 | 避雷针、引水 | 单体招式被改换目标时，当前所选目标显示 0 伤害并给出原因；范围招式不改换。Champions 中避雷针仍让持有者免疫电属性，且只吸引对手的电属性招式；引水保留双方改换目标，但不再让持有者免疫水属性 |
| 阻止招式 | 湿气、尾甲、鲜艳之躯、女王的威严 | 湿气阻止爆炸类招式；后三者阻止打向本方的先制招式，并在结果中说明原因 |
| 环境/道具压制 | 无关天气、气闸、紧张感 | 前两者让当前天气不参与伤害；紧张感让对方树果在本次计算中失效，但道具仍视为持有 |
| 特性压制 | 化学变化气体 | 压制未受特性防护护具保护且允许被压制的场上特性，同时关闭它们提供的上述联动 |

天气与场地的“设置型”特性不从当前持有者反推。日照、降雨、扬沙、降雪和场地在持有者离场后仍可能存在，App 继续以用户在战场状态中确认的当前天气/场地为准。威吓、同台共演、招待、款待、接球手/化学之力、魂心、机会主义、共生、除障等入场、受击、击倒或历史依赖特性，也必须通过当前能力等级、HP、道具和战场状态表达，不能仅凭“现在在场”重复触发。

胜利之星与无防守只改变命中率，而当前伤害区间以“招式已命中”为前提；心灵感应只处理同伴攻击。两类都不改变本模块的敌我单次伤害结果，因此不生成额外倍率。
...
- 双打范围招式样例。
- 特性和道具样例。
- 妖精气场、暗黑气场的全场同属性增伤，以及友情防守仅保护双打队友的样例。
