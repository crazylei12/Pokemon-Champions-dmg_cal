...
- 普通悬浮菜单保留 Android 的“识别双方阵容”，HUD 使用“再战”触发同一识别流程；
- 识别完成后直接在游戏/相册画面右侧打开深色 `TYPE_FLOAT` 核对面板，不返回主应用，也不新增整页复核入口；
- 面板选择已保存的我方完整队伍，并显示六个对手槽位的首选候选；每槽可从候选切换，也可按中文名、英文名或稳定 ID 搜索修正；
- 确认后原子保存识别预览与新对局并清理上一局临时状态；识别失败、空帧或保存失败时保留上一份有效状态；
- 置信度、分差和前三候选仍保留在识别域与自动化契约中，但正式产品不暴露 ROI、OCR、模板文件名或测试图片等调试概念。
...
- 实际在横屏下创建右侧悬浮核对面板，检查已保存我方队伍、对手六槽候选、搜索修正、重试和确认入口；
