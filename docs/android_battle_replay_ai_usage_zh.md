...
录屏当前优先使用以下档位；启动时会按设备 H.264 编码器能力和尺寸对齐要求协商，不能接受首选档位时自动降级：

| 项目 | 当前值 |
| --- | --- |
| 容器 | MP4 |
| 视频 | H.264/AVC |
| 首选画布 | 960 x 540，保持比例，必要时加黑边 |
| 降级画布 | 约 854 x 480 / 20 fps；需要 16 像素对齐时为 848 x 480，再不支持则约 640 x 360 / 20 fps |
| 首选帧率 | 最高 24 fps |
| 视频码率 | 首选约 1.5 Mbps，降级约 1.0 / 0.75 Mbps，并限制在编码器能力范围内 |
...
4. 授权后助手会检测 Pokémon Champions 内部声音；检测到稳定信号后立即继续，不再固定等待 2.5 秒。双声道预检最多 700 ms，失败或无信号时再用单声道最多检测 500 ms。
...
- 保存的视频通常为 960 x 540；设备编码器不支持时使用启动日志和成品轨道中记录的协商降级尺寸。给识别功能的帧保持投屏原始尺寸，不使用回放编码帧冒充识别输入。
...
- `ok=true`；
- 容器 MIME 为 `video/mp4`；
- 视频为 H.264，尺寸等于本次启动协商档位，视频样本数大于零；
...
| 画面正确 | H.264 轨道等于本次协商档位、方向正确、保持比例、不裁掉游戏 UI | 拉伸、倒置、裁切、持续黑帧 |
...
- 当前探针会同时核对黑色边框、品红/青色对角棋盘形状和可见画面，不再因为画面里只有两种相近颜色就直接拒绝；黑帧或无法确认的画面会按失败处理。
- Android 公共接口仍不能直接返回“用户选择的准确包名”，因此自检不能替代成品时间线检查。
- 不得为了绕过误报退回整屏捕获。
...
如果是“点击开始”到红点出现较慢，按同一次操作的时间戳筛选 `OverlayCaptureService` 日志。`Replay start complete` 会记录 `requestToRunningMs`、`preparationToRunningMs`、实际视频档位和最终声道数；前者可能包含用户停留在授权页的时间，后者只统计本轮准备、声音预检、编码器创建和隔离检查。
...
- 本次兼容与启动优化后的 RMX3820 Debug 实测：`requestToRunningMs=1146`、`preparationToRunningMs=1145`，双声道预检读取 14,336 个样本后提前通过，实际使用 `OMX.qcom.video.encoder.avc` 和 960×540 / 24 fps。新成品 MediaStore ID `1000012517` 为 99.733 秒、19,940,979 字节、`is_pending=0`；音频/视频首 PTS 都为 0，末 PTS 相差约 24 ms，五点帧均可解码且首帧不是黑帧。
- 同机第二次启动为 1,145 ms。录制中把游戏切到后台，使捕获画面长时间保持同一帧；成品 MediaStore ID `1000012519` 为 54.336 秒、`is_pending=0`，视频只有 614 个样本但时间线仍覆盖 54.318 秒，音频/视频末端相差约 18 ms，证明静止期间的低频保持帧和停止终帧没有让视频时长停住。
...
- 同一版 arm64 Debug APK 已在本次工作中成功覆盖安装到 OPD2409；设备当时处于安全锁屏，本轮没有把“安装成功”写成新录屏链路已经复测。
...
- H.264 启动会枚举可用编码器，优先硬件编码，并按尺寸、帧率、码率和 Surface 输入能力协商首选或降级档位。
- 视频从隔离检查缓存帧以 0 时间戳同步起步；静止画面期间会提交低频保持帧，停止时再提交终帧，避免只有音频增长而视频停在首帧。
- 双声道播放捕获在预检或正式启动失败时会重建为单声道；预检实例与正式实例保持隔离，以保留 OPD2409 / ColorOS 的 `AudioRecord -38` 修复。
- STARTING 阶段收到编码或捕获线程异常时会走启动回滚并恢复识别 Surface；只有 RUNNING 阶段异常才进入正常录屏收尾。
...
| `replay/EglProjectionRouter.kt` | 单应用帧路由、协商档位编码、首帧/静态保持帧、原分辨率按需读回和隔离探针 |
| `replay/ReplayVideoEncoder.kt` | H.264 能力枚举、档位降级、Surface 编码和视频 EOS |
