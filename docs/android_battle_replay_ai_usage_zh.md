...
适用实现：`feature/battle-replay-phase-4`，至少包含提交 `69df195`
...
5. 选择模式不等于已经开录。只有悬浮球显示红点和计时、状态提示“已开始”后，正式 MP4 时间线才开始。
6. 正常停止要使用“结束并保存 MP4”、前台通知或系统投屏芯片，并等待“回放已保存”。不要用 `force-stop` 代替正常保存。
...
| 模式 | 是否生成 MP4 | 是否加载识别和伤害功能 | 适用场景 |
| --- | --- | --- | --- |
| `仅录屏` | 是 | 否 | 只想低开销保存游戏画面和声音；也是录屏管线的首选基础验收模式 |
| `识别并录屏` | 是 | 是，按需读取原分辨率画面 | 一边使用我方队伍 OCR、双方阵容识别和伤害面板，一边保存回放 |
| `仅识别` | 否 | 是 | 只使用原有识别和伤害功能，不请求声音权限、不创建 MP4 |

模式选定后会在本次 MediaProjection 会话中锁定。要更换模式，必须结束当前会话、等待保存完成、重新授权，再从新的悬浮球菜单选择。
...
| 声道 | 优先双声道，不支持时单声道 |
| 保存目录 | `Movies/Pokemon Champions Replays/` |
| 文件名 | `pokemon-champions_yyyyMMdd_HHmmss.mp4` |

## 4. 使用前提

- 录屏首发只在 Android 16 / API 36 开放；更低系统只能使用`仅识别`。
...
6. 返回游戏后等待悬浮球出现。此时悬浮球应显示“选择模式”，还没有正式录制。
7. 点击悬浮球，选择`仅录屏`、`识别并录屏`或`仅识别`。
...
- 点击悬浮球可看到当前录制时长。
- 菜单只提供“结束并保存 MP4”。
- 不应初始化 OCR、队伍模板、识别悬浮层或伤害计算 WebView。

`识别并录屏`模式：

- 点击悬浮球后，助手会先按需冻结一帧原分辨率游戏画面，再打开菜单；不要连续重复点击。
- 菜单可包含“录入我的队伍”“识别双方阵容”“核对双方阵容并开始对局”“打开伤害面板”等入口。
- 识别读取、核对窗口和伤害面板失败时，录屏应继续。
- 保存的视频仍为 960 x 540；给 OCR 的帧保持投屏原始尺寸，不使用 540p 回放帧冒充识别输入。
...
3. 等待“回放已保存：<时长>，<大小>”；
4. 点击“对局回放已保存”通知可以直接打开视频；
5. 也可以在系统相册的 `Movies/Pokemon Champions Replays/` 中查找。
...
5. 收起面板，继续录几秒，然后正常结束并保存。
6. 检查菜单或面板实际展开的对应时间点，MP4 中只能出现游戏画面。
...
找到 `relative_path=Movies/Pokemon Champions Replays/` 的最新一行，并记录：
...
Debug APK 提供五点时间线检查器。先确保录屏服务已经停止，再对上一步的 MediaStore ID 执行：
...
adb -s $deviceSerial shell am start -S -W `
  -n $probeActivity `
...
| 正常收尾 | 保存完成后服务和投屏释放，无新 pending 垃圾 | 用强杀代替正常停止 |
| 模式边界 | `仅录屏`不出现识别入口，也不初始化识别/伤害运行时 | 仅凭 UI 名称推断内部没有加载 |
...
- 当前只有 API 36 会启用`仅录屏`和`识别并录屏`。
- Android 13～15 保留`仅识别`，不要修改系统限制强行开放录屏。
...
- 一个 MediaProjection token 内模式锁定。
- 正常结束当前会话，等待 MP4 保存，再重新授权选择新模式。

## 10. 当前已确认范围和未完成边界

截至 2026-07-17：

- RMX3820 / Android 16 已验证单应用画面隔离、有声/静音 UID 捕获、10 分钟有声回放、异常 pending 清理和`识别并录屏`大厅验收。
- 大厅验收中，悬浮菜单和伤害面板真实展开时，对应 MP4 时间点仍只有游戏画面。
- OPD2409 的同等音频、长录和最终退出结论尚未完成。
- 30 分钟稳定性、空间不足、温度监控、完整性能基线和最终 Release 产品化仍属于 Phase 5/6，不能写成已完成。
...
| `CaptureSessionMode.kt` | 三种模式、会话状态机、声音决策和模式锁定 |
| `ReplayPermissionActivity.kt` | 声音权限说明、拒绝后的显式无声选择 |
| `OverlayCaptureService.kt` | 唯一会话所有者、悬浮球菜单、模式编排、统一停止和保存通知 |
| `RecognitionFeatureHost.kt` | 只在包含识别的模式中延迟创建 OCR、模板和伤害相关组件 |
...
| `replay/ReplayMediaStore.kt` | pending 创建、正常发布、失败丢弃和遗留项清理 |
