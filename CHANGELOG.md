...
- 新增离线 PS / Showdown 队伍文本导入，以及已保存队伍和公开队伍码预览的 PS 导出，保留性别并支持复制全部文本。
- 默认使用 Champions 能力点；传统 EV 导入提供显式近似换算选项，不支持的字段会提示。
- 补齐电光双击、复生祈祷等 10 个缺失招式，按客户端核对全部 512 个开放招式和 396 个形态学习表。
- 修正坚盾剑怪、一家鼠形态编号、形态特性池、内置配置及部分中文形态名称。
- 修复庞克摇滚和化学变化气体的伤害计算联动，增加完整目录回归检查。
- 仅发布 Android 标准版 1.1.12 (17)。已有错误配置不自动猜测迁移，需重新导入并核对保存。

完整说明见 [1.1.12 发布说明](docs/release_1.1.12_zh.md)。

## [1.1.11] - 2026-09-10

- 恢复游戏 1.2.0 公开队伍码查询，每次查询重新取得完整性证明并建立临时会话。
- 无需启动游戏，可预览六只宝可梦的配置，核对后保存为我的队伍或六个对手配置。
- 查询需要可用的 Google Play 商店、服务与可连接 Google 和官方服务的网络；截图识别和离线计算仍可独立使用。
- README 和发布说明加入手机相册中的队伍码解析实机截图。
- 仅发布 Android 标准版 1.1.11 (16)。

完整说明见 [1.1.11 发布说明](docs/release_1.1.11_zh.md)。

## [1.1.10] - 2026-09-10

- 仅发布 Android 标准版 1.1.10 (15)，录屏版与 HarmonyOS 停止维护。
- 补齐 166 件 v18 道具及其中文选择、OCR 与离线编号映射。
- 修复气球和一般宝石计算；场地种子等级保留手动控制，避免重复叠加。
- 移除无效录屏按钮并收紧 HUD 顶部布局，迁移旧布局时保留其他部件位置。
- 在线队伍码查询尚未恢复，本轮未执行真机验收。

完整说明见 [1.1.10 发布说明](docs/release_1.1.10_zh.md)。

## [1.1.9] - 2026-09-09

本次仅更新和发布 Android 标准版 `1.1.9 (14)`，不维护录屏版或 HarmonyOS。

### 新增与改进

- 战斗 HUD 支持双方场上宝可梦切换形态；重算后保留当前编辑子页，长表单顶部提供“重新计算”入口。
- 计算纳入场上持续生效的特性联动与压制；需要回合历史的触发效果仍由用户确认当前状态。
- 更新九月 Champions 合法形态、招式池、道具与常用配招，以及新 Mega 形态特性。流星突刺按 170 威力计算，并补充葱游兵当前合法招式；劈开威力和部分招式基础 PP 同步更新。
- 更新 Master Data v18 数字映射及普通/闪光图标特征；补齐包括“危险回避”在内的特性中文词库，改善我方队伍两页 OCR。
- 构建工具 sharp 升级至 0.35.4，修复依赖审计发现的高危漏洞。
- 新增公开队伍码导入入口，支持保存整支我方队伍或六个对手配置；但游戏 1.2.0 在线登录当前失败，该入口暂不可正常查询。

### 已知限制

- v18 离线映射通过校验不代表在线队伍码查询恢复；当前登录返回 1001，请使用截图识别或手动配置。
- 本次发布构建的自动测试、签名和包级验证结果见发布说明。此前两页 OCR 实机记录来自独立调试包，不等同于本次正式 APK 的覆盖升级或完整对战验收。

完整说明见 [1.1.9 发布说明（Android 标准版）](docs/release_1.1.9_zh.md)。
